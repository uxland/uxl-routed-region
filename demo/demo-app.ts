...
export class DemoApp extends RegionHost(Routing(propertiesObserver(LitElement))) {
  @property()
  route: string;
  ...
  routeChanged(current: Route, previous: Route) {
    console.log('Demo - Route changed: ', current, previous);
  }
...
export class ModuleA extends RegionHost(Routing(propertiesObserver(LitElement))) {
  render() {
    return html`
      <h1>Module A</h1>
      <div>
  ...
  region: IRegion;
  routeChanged(current: Route, previous: Route) {
    console.log('Module A - Route changed: ', current, previous);
  }
  isRouteActiveChanged(current: boolean, previous: boolean): void {
    console.log('Module A - Active changed:', current, previous);
  }
...
export class ModuleB extends RegionHost(Routing(propertiesObserver(LitElement))) {
  render() {
    return html`
      <h1>Module B</h1>
      <div>
  ...
  region: IRegion;
  routeChanged(current: Route, previous: Route) {
    console.log('Module B - Route changed: ', current, previous);
  }
  isRouteActiveChanged(current: boolean, previous: boolean): void {
    console.log('Module B - Active changed:', current, previous);
  }
...
export class ItemDetail extends Routing(LitElement) {
  render() {
    return html`
      <div>
        <span>${this.params && (<any>this).params.id}</span>
